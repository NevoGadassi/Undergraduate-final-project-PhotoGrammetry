...
/**
 * Packed array of {@link Point4D_F32}. Internally the point is stored in an interleaved format.
 *
 * @author Peter Abeles
 */
@Generated("boofcv.struct.packed.PackedArrayPoint4D_F64")
public class PackedArrayPoint4D_F32 implements PackedArray<Point4D_F32> {
	...
	@Override public void append( Point4D_F32 element ) {
		array.add(element.x);
		array.add(element.y);
		array.add(element.z);
		array.add(element.w);
	...
	@Override public Point4D_F32 getTemp( int index ) {
		index *= 4;
	...
	@Override public void getCopy( int index, Point4D_F32 dst ) {
		index *= 4;
	...
	@Override public void copy( Point4D_F32 src, Point4D_F32 dst ) {
		dst.setTo(src);
	}
	...
	@Override public Class<Point4D_F32> getElementType() {
		return Point4D_F32.class;
	}

	@Override public void forIdx( int idx0, int idx1, BoofLambdas.ProcessIndex<Point4D_F32> op ) {
		int pointIndex = idx0;
		idx0 *= DOF;
		idx1 *= DOF;
